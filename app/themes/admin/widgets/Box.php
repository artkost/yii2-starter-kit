...
namespace app\themes\admin\widgets;
...
/**
 * Class Box
 * @package app\themes\admin\widgets
 * Theme Box widget.
 */
    ...
    protected function initButtons()
    {
        if (!isset($this->buttons['create'])) {
            $this->buttons['create'] = [
                ...
                'options' => [
                    'class' => 'btn-default',
                    'title' => Theme::t('widgets/box', 'Create')
                ]
            ];
        }
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = [
                ...
                'options' => [
                    'class' => 'btn-default',
                    'title' => Theme::t('widgets/box', 'Delete'),
                    'data-confirm' => Theme::t('widgets/box', 'Are you sure you want to delete this item?'),
                    'data-method' => 'delete'
                ]
            ];
        }
        ...
        if (!isset($this->buttons['batch-delete'])) {
            $this->buttons['batch-delete'] = [
                ...
                'options' => [
                    'id' => 'batch-delete',
                    'class' => 'btn-default',
                    'title' => Theme::t('widgets/box', 'Delete selected')
                ]
            ];
        }
        ...
        if (!isset($this->buttons['cancel'])) {
            $this->buttons['cancel'] = [
                ...
                'options' => [
                    'class' => 'btn-default',
                    'title' => Theme::t('widgets/box', 'Cancel')
                ]
            ];
        }
    }
        ...
        if (strpos($this->buttonsTemplate, '{batch-delete}') !== false && $this->grid !== null && isset($this->buttons['batch-delete'])) {
            ...
            $view->registerJs(
                "jQuery(document).on('click', '#batch-delete', function (evt) {" .
                    ...
                    "if (keys == '') {" .
                        "alert('" . Theme::t('widgets/box', 'You need to select at least one item.') . "');" .
                    "} else {" .
                        "if (confirm('" . Theme::t('widgets/box', 'Are you sure you want to delete selected items?') . "')) {" .
                            "jQuery.ajax({" .
                                "type: 'POST'," .
